...
// Constants - use fallback values for browser compatibility
const SUPER_AGENT_PRICING_TABLE = [
...
const GBP_TO_INR_RATE = 105.50;
const WORKER_PAY_RATE_PER_500_WORDS = 6.25;

// Only create pool in Node.js environment (server-side)
const pool = typeof window === 'undefined' ? new Pool({
...
export class PricingService {
  ...
  static async calculateAgentPrice(wordCount, agentId) {
    if (!pool) {
      // Fallback to Super Agent pricing in browser environment
  ...
  static async determinePricingType(clientId) {
    if (!pool) {
      // Default to Super Agent pricing in browser environment
